<template>
  <div
    class="de-tabs-div"
    :class="[headClass,element.label=='切换'? 'de-tabs-div-carouse':'de-tabs-div']"
  >
    <div
      v-if="maskShow"
...
export default {
  ...
  computed: {
    ...
    moveActive() {
      // console.log(this.tabMoveInActiveId, this.element.id)
  ...
  created() {
    ...
    bus.$on('chart-click', this.chartClick)
    bus.$on('fromNew', this.fromNew)
  ...
  beforeDestroy() {
    ...
    bus.$off('chart-click', this.chartClick)
    bus.$off('fromNew', this.fromNew)
  },
  methods: {
    getType() {
    ...
    chartClick(tab) {
      ...
      let res = _this.tabCanvasComponentData(_this.activeTabName)
      if (res.length == 1 && res[0].serviceName == 'textStoreHouse') {
        bus.$emit('loop')
      }
    ...
    fromNew() {
      const _this = this
      clearInterval(_this.timerTick);
      clearInterval(_this.timer);
      if (!_this.element.style.carouselEnable) {
        clearInterval(_this.timerTick);
        clearInterval(_this.timer);
      }
      _this.activeTabName = _this.element.options.tabList[0].name
      if (_this.element.style.carouselEnable) {
        const switchTime = (_this.element.style.switchTime || 5) * 1000
        let switchCount = 2
        // 轮播定时器
        _this.timerTick = setInterval(() => {
