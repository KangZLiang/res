...
export default {
  ...
  computed: {
    ...
    charViewShowFlag() {
      return this.httpRequest.status && this.chart.type && !this.chart.type.includes('table') && !this.chart.type.includes('text') && !this.chart.type.includes('battery') && this.chart.type !== 'label' && this.renderComponent() === 'echarts'
    },
    charViewG2ShowFlag() {
      return this.httpRequest.status && this.chart.type && !this.chart.type.includes('table') && !this.chart.type.includes('text') && !this.chart.type.includes('battery') && this.chart.type !== 'label' && this.renderComponent() === 'antv'
    },
    charViewS2ShowFlag() {
      return this.httpRequest.status && this.chart.type && this.chart.type.includes('table') && !this.chart.type.includes('text') && !this.chart.type.includes('battery') && this.chart.type !== 'label' && this.renderComponent() === 'antv'
    },
    ...
    labelShowFlag() {
      return this.httpRequest.status && this.chart.type && (this.chart.type.includes('text')|| this.chart.type.includes('battery'))
    },
    labelTextShowFlag() {
  ...
  mounted() {
    ...
    bus.$on('tab-data-change', this.tabDataChange)
    this.bindPluginEvent()
  },

  beforeDestroy() {
    this.innerRefreshTimer && clearInterval(this.innerRefreshTimer)
    bus.$off('tab-data-change', this.tabDataChange)
  ...
  methods: {
    ...
    clearPanelLinkage(param) {
      if (param.viewId === 'all' || param.viewId === this.element.propValue.viewId) {
        try {
          let relE = this.$refs[this.element.propValue.id];
          relE.reDrawView && (relE.reDrawView());
        } catch (e) {
