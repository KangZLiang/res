    ...
    >
      <div>
        ...
        <el-row>
          ...
          <el-col :span="14">{{ details.wareMaterielId }}</el-col>
        </el-row>
        <el-row>
...
export default {
    ...
    customStyle() {
      ...
      if (this.canvasStyleData.openCommonStyle && this.isMainCanvas()) {
        ...
        if (styleInfo.backgroundType === 'image' && typeof (styleInfo.imageUrl) === 'string') {
          style = {
            background: `url(${(styleInfo.imageUrl)}) no-repeat`
          }
  ...
  mounted() {
    ...
    bus.$on('ClickHouse', this.ClickHouse)
  ...
  beforeDestroy() {
    ...
    bus.$off('ClickHouse', this.ClickHouse)
  },
  methods: {
    beforeClose() {
      this.openDia = false
      this.addressVsible = false
      bus.$emit('beforeClose')
    },
    ClickHouse(item) {
      if(!item) return
      if(this.canvasId != 'canvas-main') return
      this.addressVsible = true
      this.headerTilte = item.wareName + '库位详情'
      axios({
        method: 'POST',
