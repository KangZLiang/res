<template>
  ...
  >
    ...
    >
      ...
      <div v-if="attrShow('carouselBg')" style="width: 20px;float: left;margin-top: 2px;margin-left: 10px;">
        <div style="width: 16px;height: 18px">
          <el-tooltip :content="$t('panel.color')">
            <i class="icon iconfont icon-zimua" @click="goBgColor" />
          </el-tooltip>
          <div :style="letterDivColor" />
          <el-color-picker ref="bgColorPicker" v-model="styleInfo.backgroundColor" style="margin-top: 7px;height: 0px" size="mini" :predefine="predefineColors" @change="styleChange" />
        </div>
      </div>
      <div
...
export default {
    ...
    styleInfo: {
      handler(newVal, oldVla) {
        if (newVal.fontSize) {
          ...
          bus.$emit('getCarouselFontSize', this.curComponent.id,newVal.fontSize)
        }
        if (newVal.fontWeight){
          bus.$emit('getCarouselFontWeight',this.curComponent.id,newVal.fontWeight)
        }
        if (newVal.color){
          bus.$emit('getCarouselFontColor',this.curComponent.id,newVal.color)
        }
      },
      deep: true
    },
  ...
  methods: {
    ...
    goBgColor(){
      this.$refs.bgColorPicker.handleTrigger()
    },
    goBoardColor() {
