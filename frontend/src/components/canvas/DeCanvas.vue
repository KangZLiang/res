...
export default {
  ...
  methods: {
    ...
    // }
    handleDrop(e) {
     let res =  e.dataTransfer.getData('chartPick')
      if(res) {
        bus.$emit('defaultData')
        bus.$emit('createChart')
      }
      ...
      if (componentInfo.type === 'assist') {
        ...
        if (this.currentWidget.filterDialog && this.currentWidget.name !== 'textStoreHouse' && this.currentWidget.name !== 'settingStoreHouse') {
          this.show = false
