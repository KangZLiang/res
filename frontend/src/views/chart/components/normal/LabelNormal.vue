<template>
  ...
  >
    ...
    <label-normal-card
      v-if="chart.type == 'text'"
      :chart="chart"
      :result="result"
      :quotaShow= "quotaShow"
      :content_class="content_class"
      :label_class="label_class"
      :dimension-show="dimensionShow"
      :label_content_class="label_content_class"
      :label_space="label_space"
    ></label-normal-card>
    <label-normal-battery
      v-if="chart.type == 'battery'"
      :chart="chart"
      :result="result"
      :quotaShow= "quotaShow"
      :content_class="content_class"
...
import LabelNormalCard from "@/views/chart/components/normal/components/LabelNormalCard";
import LabelNormalBattery from "@/views/chart/components/normal/components/LabelNormalBattery";
import {transitionString} from "@/utils/kanban";


export default {
  name: 'LabelNormal',
  components: {LabelNormalBattery, LabelNormalCard, TitleRemark, ChartTitleUpdate},
  props: {
    chart: {
      type: Object,
      required: true
  ...
  methods: {
    ...
    initStyle() {
      if (this.chart.customAttr) {
        ...
        if (customAttr.color) {
          ...
          let color = ''
          this.getResult()
          if(customAttr.color.quotaColorType) {
            let str = transitionString(customAttr.color.quotaCustomColor)
            color = new Function('result', `return ${str}`)(this.result)
          }else  {
            color= customAttr.color.quotaColor
          }
    ...
    resultFormat() {
      ...
      let color = ''
      if(customAttr.color.dimensionColorType) {
        let str = transitionString(customAttr.color.dimensionCustomColor)
        color = new Function('result', `return ${str}`)(this.result)
      }

      this.label_class.color = customAttr.color.dimensionColorType ? color : customAttr.color.dimensionColor
    },
    initRemark() {
