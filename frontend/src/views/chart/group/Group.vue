...
export default {
  ...
  created() {
    bus.$on('ganter', this.ganter)
    bus.$on('createChart', this.createChart)
    bus.$on('ChartDataList', this.ChartDataList)
  ...
  destroyed() {
    bus.$off('ganter', this.ganter)
    bus.$off('createChart', this.createChart)
    bus.$off('ChartDataList', this.ChartDataList)
  ...
  methods: {
    ...
    selectTable() {
      let domCharts = document.getElementsByClassName("draggable")
      domCharts = domCharts.length + 1
      this.selectTableFlag = true
      this.chartName = this.$t('chart.chartName') + domCharts
    },
    ...
    ChartDataList(item) {
      this.ChartDataLists = item
    },
    ganter(table) {
      this.defaultTable = table
      this.table = table
      this.chartName = '默认视图' + this.count
      this.currGroup.id = this.$store.state.panel.panelInfo.id
      this.count++
    },
    createChart() {
      ...
      view.tableId = this.table.id || this.defaultTable.id
      view.type = this.ChartDataLists.value || this.view.type
      view.isPlugin = this.view.isPlugin
      view.render = this.ChartDataLists.render || this.view.render
