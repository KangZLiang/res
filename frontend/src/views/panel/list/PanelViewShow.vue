                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      <el-dropdown-item
                        icon="el-icon-picture-outline"
                        @click.native="downloadAsImageTemplate"
                      >{{ $t('panel.export_to_sample') }}</el-dropdown-item>
                      <el-dropdown-item
                        icon="el-icon-s-data"
...
import {dataURLToBlob, getNowCanvasComponentData, imgUrlTrans} from '@/components/canvas/utils/utils'
import {findResourceAsBase64, uploadFileResult} from '@/api/staticResource/staticResource'
...
export default {
  ...
  methods: {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          html2canvas(canvasID).then(canvas => {
            ...
            document.body.removeChild(dom)
            const blob = dataURLToBlob(dom.toDataURL('image/png', 1))
            let textContain = this.transToFile(blob, this.$store.state.panel.panelInfo.id + '.png', "image/png")
            textContain.then((res) => {
              uploadFileResult(res, (reult) => {
                this.setImages(imgUrlTrans(reult))
              })
            })
            URL.revokeObjectURL(blob)
            setTimeout(() => {
    ...
    async filterImage() {
      let id = this.$store.state.panel.panelInfo.id
      let flay = true
      await selectTableDate({tableName: 'images'}).then(res => {
    ...
    async setImages(blob) {
      let res = await this.filterImage()
      let method = res ? addTableData : updateTableData
      var list = new Array();
      let row = {name: this.$store.state.panel.panelInfo.name, id: this.$store.state.panel.panelInfo.id, blob: blob}
      for (let key in row) {
        if (key !== '_XID') {
          let obj = {
            name: "",
      ...
      let data = res ? {
        ...
        "whereList": [{
          "name": "id",
          "value": this.$store.state.panel.panelInfo.id
        }]
      }
      method(data).then(result => {
        this.$message({
          message: res ? '添加成功' : '修改成功',
          type: 'success'
        })
      })
    },
