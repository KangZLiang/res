...
export default {
  ...
  mounted() {
    ...
    setTimeout(this.defaultData.bind(this),1000)
  },
  created() {
    bus.$on('defaultData',this.defaultData)
  },
  destroyed() {
    bus.$off('defaultData',this.defaultData)
  },
  methods: {
    defaultData() {
      if(this.expand) {

        const firstNode = document.querySelector('#tableTreeSelect')?.querySelector('.el-tree-node')?.querySelector('.el-tree-node');
